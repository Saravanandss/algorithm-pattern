"""Universal Sink in a directed graph (adjacency-matrix form).
...
is all zeros and column k is all ones except the diagonal. A graph has at most
one universal sink.

Key insight — O(n) elimination instead of O(n^2) scanning:
Walk from (0, 0). At (i, j):
  - graph[i][j] == 1  =>  i is not a sink (a sink's row is all zeros), advance i.
  - graph[i][j] == 0  =>  j is not a sink (a sink's column is all ones), advance j.
...
(the graph may have none), so a final O(n) verification of that candidate's
